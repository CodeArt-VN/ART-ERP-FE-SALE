...
export class SaleOrderMobilePage extends PageBase {
	...
	preLoadData(event) {
		...
		Promise.all([this.env.getStatus('SalesOrder')]).then((values: any) => {
	...
	changeSelection(i, e = null) {
		...
		this.selectedItems.forEach((i) => {
			...
			let notShowDelete = ['Submitted', 'Approved', 'Scheduled', 'Picking', 'InCarrier', 'InDelivery', 'Delivered', 'Splitted', 'Merged', 'Debt', 'Done'];
	...
	submitOrdersForApproval() {
		...
		this.alertCtrl
			.create({
				header: 'Gửi duyệt',
				//subHeader: '---',
				message: 'Sau khi gửi duyệt, bạn không thể chỉnh sửa đơn hàng được nữa. Bạn có chắc muốn gửi duyệt tất cả đơn hàng?',
				buttons: [
					{
						text: 'Không',
						role: 'cancel',
